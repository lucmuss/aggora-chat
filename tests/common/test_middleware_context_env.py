...
class TestCommonInfrastructure:
    ...
    def test_canonical_host_middleware_allows_legacy_alias_host(self, settings):
        settings.APP_PUBLIC_URL = "https://aggora.org"
        settings.ALLOWED_HOSTS = ["aggora.org", "www.aggora.org", "aggora.kolibri-kollektiv.eu"]
        middleware = CanonicalHostMiddleware(lambda request: HttpResponse("ok"))
        request = RequestFactory().get("/popular/", HTTP_HOST="aggora.kolibri-kollektiv.eu")
